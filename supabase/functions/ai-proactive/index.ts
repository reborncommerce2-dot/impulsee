...
Deno.serve(async req=>{ if(req.method==='OPTIONS')return new Response('ok',{headers:cors}); try{
 ...
 const [{data:habits},{data:logs},{data:objectives},{data:expenses},{data:consumptions}] = await Promise.all([
  admin.from('habits').select('id,name,target_value,frequency').eq('user_id',user.id).is('deleted_at',null),
  admin.from('habit_logs').select('habit_id,date,value').eq('user_id',user.id).gte('created_at',since).is('deleted_at',null),
  admin.from('objectives').select('id,title,end_date,status').eq('user_id',user.id).is('deleted_at',null),
 ...
 if(settings.goal_deadlines) for(const o of objectives||[]){ if(o.end_date&&o.status!=='completed'){const days=Math.ceil((Date.parse(o.end_date)-Date.parse(today))/86400000);if(days>=0&&days<=7)events.push({kind:'goal_deadline',title:`Tu objetivo “${o.title}” está cerca de vencer`,body:`Quedan ${days} día(s). ¿Querés que revisemos el plan?`,priority:3,source:{objectiveId:o.id}});}}
